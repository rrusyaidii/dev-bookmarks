...
export default function Sidebar({
  ...
  return (
    <>
      ...
      >
        <div
          className={`flex items-start justify-between gap-1 border-b border-border px-3 py-4 ${
            slim ? 'md:flex-col md:items-center md:px-2 md:py-4' : 'md:px-4 md:py-5'
          }`}
        >
          <Link href="/" onClick={onClose} className="min-w-0 flex-1 md:flex-none">
          ...
          {/* Phone: close drawer. Desktop: collapse rail (top — never buried). */}
          <button
            type="button"
            onClick={onClose}
            className="flex size-10 shrink-0 cursor-pointer items-center justify-center text-muted transition-colors hover:text-fg md:hidden"
          ...
          <button
            ...
            className="hidden size-10 shrink-0 cursor-pointer items-center justify-center text-muted transition-colors hover:text-fg md:flex"
            aria-label={slim ? 'Expand sidebar' : 'Collapse sidebar'}
            title={slim ? 'Expand' : 'Collapse'}
          >
            <CollapseIcon expanded={!slim} />
          </button>
        </div>
